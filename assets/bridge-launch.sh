...
# detached `claude -p` process that owns the AUTOMATION RESILIENCE CONTRACT
# itself, so begin/checkpoint/side-effect/finish run inside the long-lived
# process rather than inside a Zo turn that has already returned.
#
...
#
# Usage:
#   bridge-launch.sh --job NAME --prompt-file PATH [--preflight CMD] [--workdir DIR]
#
...
JOB=""; PROMPT_FILE=""; PREFLIGHT=""; WORKDIR="/home/workspace"
while [ $# -gt 0 ]; do
  case "$1" in
    --job) JOB="$2"; shift 2 ;;
...
# Resolve the detached runner: explicit override, then a sibling copy installed by
# this kit, then the legacy zo-swarm-orchestrator location. Resolving a sibling first
# keeps an installed pair self-contained on a host that has no swarm orchestrator.
...
exec "$RUNNER" "$(cat "$PROMPT_FILE")" "$JOB" "$WORKDIR"
